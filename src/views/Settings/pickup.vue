    ...
    <template #content>
      <div class="bg-gray-200 h-auto">
        <div class="container px-4 py-10 mx-auto max-w-7xl">
          ...
          >
            <!-- Custom Field -->
            <template slot="pickup_name" slot-scope="props">
              <div class="flex item-center justify-between">
                <div class="flex items-center">
                  {{ props.item.pickup_name }}
                </div>
                <div class="flex items-center">
                  <button
                    @click="editPickup(props.item.id)"
        ...
        >
          <addForm
            ...
            :title="dataSelected.length === 0 ? 'Tambah Pickup' : 'Ubah Pickup'"
            @submit="handleSubmit"
          >
            <div class="space-y-4">
              <div>
          ...
          <notification
            ...
            :message="
              dataSelected.length === 0
                ? 'Berhasil! Pickup baru sudah ditambahkan!'
                : 'Berhasil! Pickup sudah diubah!'
            "
        ...
        >
          <modal
            v-show="deleteConfirmModal"
            title="Hapus Data Pickup"
            message="Anda yakin ingin menghapus data pickup ini?<br/> Data akan dihapus secara permanen. Tindakan ini tidak bisa dibatalkan."
            @close="
              deleteConfirmModal = false;
              dataSelected = [];
            "
            @confirm="deletePickup()"
          ></modal>
        </transition>
...
export default {
  ...
  methods: {
    ...
    async editPickup(id) {
      try {
        const response = await axios.get(
          `${api_url}/api/v1/master/pickup/get/${id}`,
          { headers: this.headers }
        );
        console.log(response);
        ...
        /* Set Data Form */
        this.pickup_name = this.dataSelected.pickup_name;
        this.pickup_phone = this.dataSelected.pickup_phone;
        this.branch = this.dataSelected.branch.map((item) => {
          return {
            label: item.label,
            value: item.value,
          };
    ...
    async updatePickup() {
      ...
      try {
        const response = await axios.put(
          `${api_url}/api/v1/master/pickup/update/${this.dataSelected.id}`,
          data,
          { headers: this.headers }
        );
        console.log(response.data);
    ...
    async deletePickup() {
      try {
        const response = await axios.delete(
          `${api_url}/api/v1/master/pickup/delete/${this.dataSelected.id}`,
          { headers: this.headers }
        );
        console.log(response.data);
