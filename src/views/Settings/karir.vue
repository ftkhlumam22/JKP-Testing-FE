      ...
      <div class="bg-gray-200 h-auto">
            ...
            <div class="space-y-4">
              ...
              <div>
                <label
                  ...
                  class="block text-sm leading-5 font-semibold text-gray-800 mb-1"
                >
                  Job Desc/Deskripsi Pekerjaan
                </label>
                <vue-editor v-model="job_desc"></vue-editor>
              </div>
              <div>
                <label
                  id="listbox-label"
        ...
        >
          <notification
            ...
            :message="
              dataSelected.length === 0
                ? 'Berhasil! Lowongan Kerja baru sudah ditambahkan!'
                : 'Berhasil! Lowongan Kerja sudah diubah!'
            "
        ...
        >
          <modal
            v-show="deleteConfirmModal"
            title="Hapus Data Lowongan Kerja"
            message="Anda yakin ingin menghapus data lowongan kerja ini?<br/> Data akan dihapus secara permanen. Tindakan ini tidak bisa dibatalkan."
            @close="
...
export default {
  ...
  methods: {
    ...
    async getListData(page, limit) {
      ...
      try {
        const response = await axios.get(`${api_url}/api/v1/master/job/admin`, {
          params: query_params,
    ...
    async createJob() {
      ...
      try {
        const response = await axios.post(
          `${api_url}/api/v1/master/job/create`,
          data,
          { headers: this.headers }
        );
    ...
    async editJob(id) {
      try {
        const response = await axios.get(
          `${api_url}/api/v1/master/job/get/${id}`,
          { headers: this.headers }
        );
        console.log(response);
        ...
        /* Set Data Form */
        this.position = this.dataSelected.position;
        this.job_desc = this.dataSelected.job_desc;
        this.requirement = this.dataSelected.requirement;
        this.location = this.dataSelected.location;
        this.job_type = this.dataSelected.job_type;
        this.experience = this.dataSelected.experience;
        this.start_date = this.dataSelected.start_date;
        this.end_date = this.dataSelected.end_date;
        this.misc_info = this.dataSelected.misc_info;
      } catch (error) {
        console.log(error);
      }
    },
    async updateJob() {
      ...
      try {
        const response = await axios.put(
          `${api_url}/api/v1/master/job/update/${this.dataSelected.id}`,
          data,
          { headers: this.headers }
        );
        console.log(response.data);
    ...
    async deleteJob() {
      try {
        const response = await axios.delete(
          `${api_url}/api/v1/master/job/delete/${this.dataSelected.id}`,
          { headers: this.headers }
        );
        console.log(response.data);
