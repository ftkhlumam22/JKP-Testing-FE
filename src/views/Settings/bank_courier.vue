      ...
      <div class="bg-gray-200 h-auto">
        ...
        >
          <addForm
            ...
            :title="dataSelected.length === 0 ? 'Tambah Bank' : 'Ubah Bank'"
            @submit="handleSubmit"
          >
            <div class="space-y-4">
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  class="w-full rounded-md border-2 border-gray-300 focus:outline-none focus:border-blue-800 text-sm px-3 py-2"
                  placeholder="contoh: 13455944333"
                  type="text"
                />
              </div>
            </div>
        ...
        >
          <modal
            ...
            message="Anda yakin ingin menghapus data bank ini?<br/> Data akan dihapus secara permanen. Tindakan ini tidak bisa dibatalkan."
            @close="
...
export default {
  ...
  methods: {
    ...
    async getListData(page, limit) {
      ...
      try {
        const response = await axios.get(
          `${api_url}/api/v1/master/bank-courier/admin`,
    ...
    async createBank() {
      ...
      try {
        const response = await axios.post(
          `${api_url}/api/v1/master/bank-courier/create`,
          data,
    ...
    async editBank(id) {
      try {
        const response = await axios.get(
          `${api_url}/api/v1/master/bank-courier/get/${id}`,
          { headers: this.headers }
        );
        console.log(response);
        ...
        /* Set Data Form */
        this.bank_name = this.dataSelected.bank_name;
        this.account_name = this.dataSelected.account_name;
        this.account_number = this.dataSelected.account_number;
      } catch (error) {
        console.log(error);
      }
    },
    async updateBank() {
      ...
      try {
        const response = await axios.put(
          `${api_url}/api/v1/master/bank-courier/update/${this.dataSelected.id}`,
          data,
          { headers: this.headers }
        );
        console.log(response.data);
    ...
    async deleteBank() {
      try {
        const response = await axios.delete(
          `${api_url}/api/v1/master/bank-courier/delete/${this.dataSelected.id}`,
          { headers: this.headers }
        );
        console.log(response.data);
