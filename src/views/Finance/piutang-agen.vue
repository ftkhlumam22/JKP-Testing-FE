      ...
      <div class="bg-gray-200 h-auto">
        <div class="container px-4 py-10 mx-auto max-w-7xl">
          <div class="flex item-center space-x-4 mb-8">
            ...
            >
              <h4 class="text-base font-medium mb-2">Piutang Agen Bulan Ini</h4>
              <h1 class="text-xl font-semibold text-blue-800">
                {{
                  formatMoney(
                    total_month_paid.total_amount -
                      total_month_paid.total_paid_cash -
                      total_month_paid.total_paid_transfer
                      ? total_month_paid.total_amount -
                          total_month_paid.total_paid_cash -
                          total_month_paid.total_paid_transfer
                      : 0
            ...
            >
              ...
              <h1 class="text-xl font-semibold text-blue-800">
                {{ total_month_transaksi ? total_month_transaksi : 0 }}
              </h1>
            </div>
          </div>
          <div class="flex items-center justify-between mb-6">
            <div class="lg:w-1/2 w-full flex">
              <input
                ...
                type="text"
                class="border mr-4 border-gray-400 px-3 py-2 text-sm rounded-md lg:w-1/2 w-full"
            ...
            <div class="lg:w-1/2 w-full">
              <div class="flex justify-between px-3">
                <button
                  ...
                  class="bg-green-600 pl-4 hover:bg-green-700 px-3 py-2 text-sm font-semibold text-white rounded-md focus:outline-none mr-2 flex items-center ml-auto"
                >
                  <svg
          ...
          >
            ...
            <template slot="notpaid" slot-scope="props">
              {{
                props.item.transaction.filter(
                  (item) => item.payment_status === "Belum Bayar"
                ).length
              }}
            </template>
            ...
            <template slot="total_nominal" slot-scope="props">
              <!-- {{ 'Rp' + formatPrice( props.item.transaction.reduce((total, num) => total + parseInt(num.amount_paid !== null && num.total_paid_cash && num.total_paid_transfer ? num.amount_paid - (num.total_paid_cash && num.total_paid_transfer ? num.total_paid_cash + num.total_paid_transfer : 0) : 0), 0) ) }} -->
              {{ "Rp. " + formatPrice(totalNominal(props.item.transaction)) }}
            </template>
            <template slot="action" slot-scope="props">
              <div class="flex items-center">
                <button
                  @click="detailPiutangAgen(props.item)"
                ...
                <button
                  @click="bayarPiutangAgen(props.item)"
                  class="bg-gray-200 hover:bg-gray-300 border border-gray-300 hover:border-gray-400 focus:border-gray-400 rounded-md px-3 py-1 text-sm text-gray-600 focus:outline-none flex items-center mr-2"
                  style="font-size: 11px"
                >
                  <svg
        ...
        >
          ...
          >
            ...
            >
              <!-- <h1>Total yang harus dibayar: <span class="font-semibold text-green-700 text-sm">{{ 'Rp' + formatPrice( selectedItem.reduce((total, num) => total + parseInt(num.amount_paid), 0) ) }}</span></h1> -->
              <h1>
              ...
              <!-- <button class="bg-green-500 hover:bg-green-600 text-white px-3 py-1.5 font-medium focus:outline-none rounded-md text-sm">Bayar Sekarang</button> -->
            </div>
            ...
            <div class="p-6">
              ...
              <div>
                ...
                <input
                  ...
                  class="hidden"
                  type="file"
                  @change="importPiutangAgen($event)"
                />
                <label
                  for="upload-csv"
                  type="button"
...
export default {
  ...
  methods: {
    ...
    async getTotalAgen() {
      ...
      try {
        let response = await axios.get(
          `${api_url}/api/v1/finance/piutang-agen/count`,
          {
            headers: this.headers,
            params: query_params,
          }
        );
        console.log("data agen bulan :", response.data);

        this.total_month_paid =
          response.data.data.length > 0 ? response.data.data[0] : 0;
        this.total_month_transaksi = response.data.data[0].total_transaction;
      } catch (error) {
        console.log(error);
      }
    },
    async getTotalAgenAll() {
      try {
        let response = await axios.get(
          `${api_url}/api/v1/finance/piutang-agen/count-all`,
          {
            headers: this.headers,
          }
        );
        ...
        console.log("data agen", response.data.data);
      } catch (error) {
        console.log(error);
      }
    },
    ...
    async getListData(page, limit) {
      ...
      try {
        const response = await axios.get(
          `${api_url}/api/v1/finance/piutang-agen`,
          { params: query_params, headers: this.headers }
        );
        console.log("piutang Agen", response.data.data);
    ...
    /*
     * Detail & Bayar Piutang Agen
     */
    async detailPiutangAgen(item) {
      this.dataSelected = item;
      this.dataSelectedType = "detail";
      this.detailModal = true;
    },
    async bayarPiutangAgen(item) {
      this.dataSelected = item;
      this.dataSelectedType = "bayar";
      this.detailModal = true;
    },
    /*
     * Aksi Bayar Piutang Agen
     */
    async bayarSekarang() {
      let pushData = [];
      ...
      try {
        const response = await axios.post(
          `${api_url}/api/v1/finance/piutang-agen/pay`,
          pushData,
          { headers: this.headers }
        );
        console.log(response);
    ...
    selectItem() {
      this.selectedItem = [];
      console.log(this.dataSelected.transaction);

      if (!this.selectAllItem) {
        for (let i in this.dataSelected.transaction) {
          // if (
          //   this.dataSelected.transaction[i].amount_paid <
          //   this.dataSelected.transaction[i].total_paid_cash +
          //     this.dataSelected.transaction[i].total_paid_transfer
          // ) {
          //   this.selectedItem.push(this.dataSelected.transaction[i]);
          // }
          this.selectedItem.push(this.dataSelected.transaction[i]);
        }
      }
    ...
    async exportPiutangAgen() {
      ...
      try {
        const response = await axios.get(
          `${api_url}/api/v1/finance/piutang-agen/export`,
          { params: query_params, headers: this.headers }
        );
        ...
        return fileDownload(
          response.data,
          `PIUTANG AGEN ${this.branch + "-" + this.agen}.csv`
        );
      } catch (error) {
        console.log(error);
      }
    ...
    async importPiutangAgen(event) {
      ...
      try {
        const response = await axios.post(
          `${api_url}/api/v1/finance/piutang-agen/import`,
          data,
          { headers: this.headers }
        );
        console.log(response.data);
