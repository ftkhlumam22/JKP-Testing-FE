    ...
    <template #content>
      <div class="bg-gray-200 h-auto">
        <div class="container px-4 py-10 mx-auto max-w-7xl">
          ...
          >
            <!-- Custom Field -->
            <template slot="cost_category_name" slot-scope="props">
              <div class="flex item-center justify-between">
                <div class="flex items-center">
                  {{ props.item.cost_category_name }}
                </div>
                <div class="flex items-center">
                  <button
          ...
          <addForm
            ...
            :title="
              dataSelected.length === 0
                ? 'Tambah Kategori Operasional'
                : 'Ubah Kategori Operasional'
            "
            @submit="handleSubmit"
          >
            <div class="space-y-4">
...
export default {
  ...
  methods: {
    ...
    async createCategory() {
      let data = {
        cost_category_name: this.cost_category_name,
      };

      try {
        await axios.post(`${api_url}/api/v1/master/cost-category`, data, {
          headers: this.headers,
        });
        /* Set Toggle */
    ...
    async editCategory(id) {
      try {
        const response = await axios.get(
          `${api_url}/api/v1/master/cost-category/${id}`,
          { headers: this.headers }
        );
        ...
        /* Set Data Form */
        this.cost_category_name = this.dataSelected.cost_category_name;
      } catch (error) {
        console.log(error);
      }
    },
    async updatePickup() {
      ...
      try {
        await axios.put(
          `${api_url}/api/v1/master/cost-category/${this.dataSelected.id}`,
          data,
          { headers: this.headers }
        );
